...
function isArgumentRecord(value: unknown): value is object {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

/**
 * Reads one bounded security-relevant property without invoking accessors or
 * traversing attacker-controlled prototype state. Proxy trap failures are
 * converted to an explicit fail-closed result.
 */
function readOwnDataProperty(target: object, key: string): OwnDataPropertyResult {
