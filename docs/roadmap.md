...
- [x] `M4-045 P0` no raw secret in audit。 **ACCEPTED：Spec 0049 + DAP-001..036 + owned bounded JCS/SHA-256 digest + closed `observeAudit` projection/delivery + exact rc5 ordinary/audit coexistence；implementation/conformance head `19fe2261...`, CI #625 / Harness #567 PASS；验收记录 `docs/acceptance/m4-045-acceptance-audit.md` at `33ecd98c...`, CI #626 / Harness #568 PASS。This proves the owned Adapter audit-egress profile; it does not relabel native Harness history/ordinary observation as audit-safe or implement M5 detector/storage/retention.**
...
- [x] `M4-052 P0` document that v0.1 is not plugin sandbox。 **ACCEPTED：Spec 0052 + PSDB-001..024 + README/architecture explicit v0.1 non-sandbox boundary + dedicated source-conformance；final reviewed head `eca1b63f...`, CI #643 / Harness #585 PASS；验收记录 `docs/acceptance/m4-052-acceptance-audit.md` at `3e2cd73d...`, CI #644 / Harness #586 PASS。Documentation/security boundary only；no arbitrary in-process plugin isolation or process-isolation claim.**
