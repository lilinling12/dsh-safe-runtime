...
| Workflow seam | `dsh-workflow` is a Product-stable optional capability at `ctx.workflowEngine`; `WorkflowEngine.start()` returns a holder-owned live run and publishes observe-only lifecycle events | adapter may observe this seam only through its public contract; worker implementation details do not define safe-runtime semantics |
